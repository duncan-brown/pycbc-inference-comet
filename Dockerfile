...
RUN curl -L https://github.com/SchedMD/slurm/archive/slurm-14-11-11-1.tar.gz | tar -C /tmp -zxvf - && \
    cd /tmp/slurm-slurm-14-11-11-1 && ./configure && make -j 8 install && \
    rm -rf /tmp/slurm-slurm-14-11-11-1

# Mount points for comet
RUN mkdir -p /oasis /scratch /projects /usr/lib64/slurm /var/run/munge

# Install a recent PyCBC
RUN pip install git+git://github.com/gwastro/pycbc.git#egg=PyCBC
